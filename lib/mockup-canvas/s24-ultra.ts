...
export function drawS24UltraMockup(context: MockupCanvasContext): MockupDrawResult {
  ...
  const buttonWidth = 18 * headerScale;    
  const buttonRadius = 4 * headerScale;   

  const cameraTop = 22 * headerScale;       
  const cameraSize = 34 * headerScale;      
  const cameraLensSize = 10 * headerScale;

  const statusBarHeight = 110 * headerScale;
  const statusBarPaddingX = 56 * headerScale;
  const timeFontSize = 32 * headerScale;

  const batW = 54 * headerScale;
  const batH = 26 * headerScale; 
  const box5GSize = 36 * headerScale;
  const text5GSize = 18 * headerScale;
  const wifiSize = 30 * headerScale;
  const gap = 16 * headerScale;

  const navBarHeight = 85 * headerScale;
  const navIconSize = 28 * headerScale;
  const navGap = 85 * headerScale;
  ...
  ctx.fillText("12:30", screenX + statusBarPaddingX, statusYCenter + (2 * headerScale)); 
  ctx.restore();
  ...
  drawRoundedRectPath(ctx, currentRightX, statusYCenter - batH / 2, batW, batH, 4 * headerScale);
  ctx.stroke();
  
  ctx.fillStyle = iconBoxBg;
  const pad = 2 * headerScale;
  drawRoundedRectPath(ctx, currentRightX + pad, statusYCenter - batH / 2 + pad, batW - pad * 2, batH - pad * 2, 2 * headerScale);
  ctx.fill();
  ctx.restore();
  ...
  ctx.save();
  ctx.fillStyle = iconBoxBg;
  drawRoundedRectPath(ctx, currentRightX, statusYCenter - box5GSize / 2, box5GSize, box5GSize, 4 * headerScale);
  ctx.fill();
  ctx.font = `bold ${text5GSize}px "Inter", sans-serif`;
  ctx.fillStyle = iconBoxText;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText("5G", currentRightX + box5GSize / 2, statusYCenter + (1 * headerScale));
  ctx.restore();
  ...
  const rightIconX = navCenterX + navGap + (6 * headerScale);
